...
PSInput VertexShaderFunction(VSInput VS)
{
    ...
    float3 NormalCl = float3(0,VS.Normal.yz);
    ...
    float2 Nn = NormalCl.xyz/(length( float3( NormalCl.xy * sNorFac, NormalCl.z)));
    float2 Vn = float2(TexCoord.x, TexCoord.y);
    PS.TexCoord = reflect(Vn.xy, Nn.xy);
