...
function toggleDriveby()
	...
	--If he's arrested currently
	if getElementData(localPlayer, "arrested") or getElementData(localPlayer, "Jailed") == "Yes" then return end
...
function switchDrivebyWeapon(key,progress)
	...
	-- Get wanted for driveby threat
	if getPlayerTeam(localPlayer) and getPlayerTeam(localPlayer) ~= getTeamFromName("Government") and
		getPlayerTeam(localPlayer) ~= getTeamFromName("Staff") then
		exports.GTWwanted:setWl(0.1, 20, "You committed the crime of driveby")
	end
end
